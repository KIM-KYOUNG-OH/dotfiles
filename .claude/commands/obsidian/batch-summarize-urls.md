...
1. **문서 정책 준수**: 서브 에이전트가 문서 생성 시 `~/.claude/policies/obsidian-document-policy.md` 정책(frontmatter 구조, 태그 체계, 파일명 규칙, 저장 경로)을 따르도록 지시
2. **3개씩 병렬 처리**: 한 번에 3개 이상 동시 처리 금지
3. **즉시 업데이트**: 배치 완료 시마다 계획 파일 업데이트 필수
4. **경로 안내**: 계획 파일 생성 시 반드시 경로와 재개 명령 출력
5. **에러 기록**: 실패 시 사유를 계획 파일에 기록
