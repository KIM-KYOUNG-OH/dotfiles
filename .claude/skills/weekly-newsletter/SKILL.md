...
| coffee-time | `~/OneDrive/my-obsidian-vault/coffee-time` |
| dailies | `~/OneDrive/my-obsidian-vault/notes/dailies/` |
| newsletters | `~/OneDrive/my-obsidian-vault/newsletters/` |
...
## 경로
- Daily Notes: ~/OneDrive/my-obsidian-vault/notes/dailies/

## 실행 단계
1. Bash로 해당 주 Daily Notes 찾기 (macOS 호환):
   find ~/OneDrive/my-obsidian-vault/notes/dailies -name "*.md" -type f \
...
1. Bash로 해당 주 coffee-time 노트 찾기 (macOS 호환):
   find ~/OneDrive/my-obsidian-vault/coffee-time -name "*.md" -type f \
     -exec stat -f "%Sm %N" -t "%Y-%m-%d" {} \; 2>/dev/null | \
     awk -v start="{SATURDAY}" -v end="{FRIDAY}" '$1 >= start && $1 <= end {print $2}'
...
1. Bash로 해당 주에 수정된 .md 파일 찾기 (macOS 호환):
   find ~/OneDrive/my-obsidian-vault \( -path "*/001-INBOX/*" -o -path "*/003-RESOURCES/*" -o -path "*/000-SLIPBOX/*" \) \
